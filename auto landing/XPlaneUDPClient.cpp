...
namespace autolanding_lmz {
	...
	XPlaneUDPClient::XPlaneUDPClient(std::string _server, uint16_t _port,
		...
		int timeout = 1000; //1s  &timeout,sizeof(timeout) 要低于发送REFF的频率
	...
	void XPlaneUDPClient::listenerThread() {
		...
		while (!quitFlag) {
			...
			if ((recv_len = recvfrom(sock, (char *)buf, sizeof(buf), 0,
			...
			if (recv_len >= 4 && buf[0] == 'R' && buf[1] == 'R' && buf[2] == 'E'
				...
				for (int idx = 5; idx < recv_len; idx += 8) {
					...
					//				 cerr << " idx is " << idx << " en is " << en << " value is " << value << endl;
	...
	void XPlaneUDPClient::subscribeIndividualDataRef(std::string dataRef,
		...
		if (needToSubscribe) {
			...
			int res = sendto(sock, buf, sizeof(buf), 0,
				(struct sockaddr *) &serverAddr, slen);
	...
	void XPlaneUDPClient::sendCommand(std::string cmd) {
		int length = cmd.size() + 6;

		char* buf = new char[length];
		memset(buf, 0, length); //对于new的空间 用sizeof取大小会出错

		strcpy_s(buf, 5, "CMND");
		strcpy_s(buf + 5, strlen(cmd.c_str()) + 1, cmd.c_str());

		sendto(sock, buf, length, 0, (struct sockaddr *) &serverAddr, slen);
	...
	void XPlaneUDPClient::setDataRef(std::string dataRef, float value) {
		...
		strcpy_s(buf + 9, strlen(dataRef.c_str()) + 1, dataRef.c_str());

		sendto(sock, buf, sizeof(buf), 0, (struct sockaddr *) &serverAddr,
			slen);
	...
	void XPlaneUDPClient::setDataRefString(std::string dataRef, std::string value) {
		...
		if (regex_match(dataRef, matches, r)) {
			...
			for (int i = startIdx; i < stopIdx; i++) {
				...
				memcpy(buf + 5, &data, 4);
				memset(buf + 9, ' ', sizeof(buf) - 9);
				strcpy_s(buf + 9, strlen(dref.str().c_str()) + 1, dref.str().c_str());
